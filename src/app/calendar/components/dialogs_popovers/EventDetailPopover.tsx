...
const EventDetailPopover: React.FC<EventDetailPopoverProps> = ({
  ...
  const handleSaveEvent = (data: EventFormData & { id?: string }) => {
    if (!event || !data.id) {
      // Ensure event and id exist for an update
      setFormError("Cannot update event: Missing event ID.");
      return;
    }
    // Construct the payload expected by the update mutation
  ...
  return (
    <Popover open={isOpen} onOpenChange={onOpenChange}>
      ...
      <PopoverContent
        ...
        align="start"
        sideOffset={10}
        className="bg-card text-card-foreground border-border/50 z-50 w-80 p-0 shadow-2xl md:w-96"
        onInteractOutside={(e) => {
        ...
        {isEditing ? (
          <div className="p-4">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="font-serif text-lg font-semibold">Edit Event</h3>
              <Button
                variant="ghost"
                size="icon"
                className="h-7 w-7"
          ...
          <>
            <div
              className={cn("border-border/30 border-b p-4 pb-3", event.color)}
            >
              <div className="flex items-start justify-between">
                <h3
                  className={cn("mb-1 font-serif text-xl font-bold text-white")}
                >
                  {event.title}
                </h3>
